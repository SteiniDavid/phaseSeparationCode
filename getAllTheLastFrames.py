...
try:
    os.makedirs('testLFPICS')
except OSError as exception:
    if exception.errno != errno.EEXIST:
        raise
files = os.listdir('testLFGSDS')
...
for fil in files:
    ...
    node = import_file('testLFGSDS/'+infile, multiple_frames = False)      # load file as trajectory
    node.add_to_scene()
    print("add_to_scene")
    ...
    while a == 0:
        rs = RenderSettings(
            filename = "testLFPICS/final_tstep_" + outfile + ".png",
            size = (2000,2000),
            #generate_alpha = True,
        ...
        if a == 1:
            ...
            anim.save("testLFPICS/final_tstep_" + outfile + ".png") 
